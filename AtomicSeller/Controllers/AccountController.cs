...
namespace AtomicSeller.Controllers
{
    public class AccountController : BaseController
    {
        ...
        public IActionResult Login(LoginViewModel model, string returnUrl)
        {
            ...
            try {
                user = new User(); // db.User.FirstOrDefault(u => u.UserName == model.Login && u.Password == model.Password); 
            }
            catch (Exception ex)
            {
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public IActionResult MerchantLogin(LoginViewModel model, string returnUrl)
        {
            // Si l'une des infos n'est pas donnée
            if (!ModelState.IsValid)
            {
            ...
            try
            {
                user = new User(); // db.User.FirstOrDefault(u => u.UserName == model.Login && u.Password == model.Password); // @TODO crypto                    
            }
            catch (Exception ex)
            {
                Flash(new FlashMessage(ex.Message + " " + ex.InnerException + "\n" + ex.StackTrace, FlashMessageType.Error));
            ...
            else
            {
                ...
                // Par défaut, redirection vers la page d'index
                return RedirectToAction("MerchantProductsIndex", "Products");
            }
        }

        public string ConnectUser(string Login=null, string Password=null, string Token = null)
        {
            ...
            try
            {
                    ...
                    {
                    user = new User();//db.User.FirstOrDefault(u => u.UserName == Login && u.Password == Password);
                    }
                    else
        ...
        [HttpGet]
        [AllowAnonymous]
        public IActionResult GotoOpenAccount()
        {
            string URL = "www.atomicseller.com";
            return Redirect(URL);
        }
